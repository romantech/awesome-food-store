...
export const bodyScrollLock = target => {
  // reference1: https://markus.oberlehner.net/blog/simple-solution-to-prevent-body-scrolling-on-ios/
  // reference2: https://ishadeed.com/article/prevent-scroll-chaining-overscroll-behavior/
  // position: fixed 를 적용하면 모달창이 화면을 채우지 못하는 경우도 있어서 제외함(可能因为antd modal)
  const container = document.querySelector(target);
  return {
    enable() {
      container.style.touchAction = 'none';
      container.style.overflow = 'hidden';
      container.style.WebkitOverflowScrolling = 'none';
